...
* value
  * `boolean`
    * if `true` == `{ type: 'memory' }`
    * by default,
      * `true` | [`development` mode](/configuration/mode/#mode-development)
      * `false` | [`production` mode](/configuration/mode/#mode-production)
  * `object`
* cache the generated
  * webpack modules &
  * chunks
...
* `string: 'memory' | 'filesystem'`
  * if `memory` -> NOT allow additional configuration
  ...
  * 👁️if `filesystem` -> allow additional configuration 👁️
