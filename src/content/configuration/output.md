...
* `output`
  * == object / keyS -- specify -- how & where, output your
    * bundles,
    * assets,
    * ANYTHING / you bundle or load -- with -- webpack
...
* allows
  * specifying EACH output bundle's name /
    * path -- is specified by -- [`output.path`](#outputpath)
    * you can ALSO create a folder structure
      * _Example:_ `'js/[name]/bundle.js'`
    * ⚠️-- ONLY affects -- output files / INITIALLY loaded ⚠️
      * ❌== NOT affect
        * output files / on-demand-loaded chunks -> see [`output.chunkFilename`](#outputchunkfilename) ❌
        * files -- created by -- loaders -> see specific loader's available❌
* ALLOWED values
  * `string`
  * `function (pathData, assetInfo) => string`
* 👀recommendations 👀
  * if you use 1! [`entry`](entry-context.md#entry) point -> use a static name
    ...
    ```
  * if you create MULTIPLE bundles ( -- via -- >1 entry point OR code splitting OR various plugins) -> give UNIQUE name / EACH bundle
    * ways
      * entry name
        ```javascript, title=webpack.config.js
        module.exports = {
      ...
      * chunk id
        ...
        ```
      * hashes / -- generated from the -- generated content
        ```javascript, title=webpack.config.js
        module.exports = {
          //...
          output: {
        ...
        ```
      * combine MULTIPLE PREVIOUS approaches
        ```javascript, title=webpack.config.js
        module.exports = {
          //...
          output: {
...
* `string = 'var'`
* allows
  * configuring how the library -- will be -- exposed
* recommendations
  * 👀use [`output.library.type`](#outputlibrarytype)👀
    * Reason: 🧠we might drop support for `output.libraryTarget` 🧠
* requirements
  * use ALSO [`output.library`](#outputlibrary)

### ALLOWED values -- based on -- use cases

* _NEXT Examples:_ assume
  * [`output.library: MyLibrary`](#outputlibrary)
  * `_entry_return_` == value / -- returned by the -- entry point
    * == function's output / from the entry point -> -- generated by -- webpack
...
The **return value of your entry point** will be assigned to the `exports` object using the `output.library` value.
...
* 👀-> OUTPUT bundle has a COMPLETE header / -- ensure compatibility with -- various module systems 👀
...
* recommendations
  * 👀use BETTER [`output.library.type: 'umd'`](#type-umd)👀
* allows
  * 💡exposing your library | ALL module definitions 💡
    * -> -- allowing it to work with --
      * CommonJS,
      * AMD
      * global variable
* see [UMD](https://github.com/umdjs/umd)
* requirements
  * `output.library`
    * OTHERWISE, see [object assignment section](#expose-via-object-assignment)
* _Examples:_
  * _Example1:_
    ```javascript
    module.exports = {
    ...
    ```
  * _Example2:_ if you do NOT specify `output.library` -> ALL properties / returned by the entry point -- will be assigned DIRECTLY to the -- root object
    ```javascript
    module.exports = {
      //...
      output: {
    ...
    ```
* | webpack 3.1.0+,
  * you may specify `library` == object / DIFFERENT names per targets
    * _Example:_
      ```javascript
      module.exports = {
        //...
